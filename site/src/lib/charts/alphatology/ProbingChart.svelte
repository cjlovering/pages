...
<script>
	...
	function render() {
		...
		// Layer scale for dots (right axis)
		...
		g.selectAll('.bar').data(items).enter().append('rect')
			...
			.attr('fill', d => theme.conceptColors[d.concept] ?? '#999')
			.attr('rx', 1)
			.attr('opacity', 0.85);
		...
		// Best-layer dots (on right y-axis)
		...
		leg.append('rect').attr('width', 10).attr('height', 10).attr('rx', 1)
			.attr('fill', theme.ink[5]).attr('opacity', 0.5);
